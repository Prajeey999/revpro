...
const REQUIRED_TIER_ID = String(
  process.env.PATREON_REQUIRED_TIER_ID || "",
).trim();
...
// Chrome extension flow (unchanged)
...
// Mobile app flow — uses its own redirect URI so no state param needed
app.get("/auth/patreon/mobile", authLimiter, (req, res) => {
  const redirectUri = encodeURIComponent(process.env.PATREON_REDIRECT_URI_MOBILE);
  const url = `https://www.patreon.com/oauth2/authorize?response_type=code&client_id=${process.env.PATREON_CLIENT_ID}&redirect_uri=${redirectUri}&scope=identity%20identity.memberships%20identity[email]`;
  res.redirect(url);
});

// Chrome extension callback — returns postMessage HTML page
app.get("/auth/patreon/callback", async (req, res) => {
  ...
  try {
    ...
    const memberships = (userData.included || []).filter(
      (m) => m.type === "member",
    );
    ...
    const isMember = memberships.some((m) => {
      ...
      return (
        tierIds.includes(REQUIRED_TIER_ID) &&
        (status === "active_patron" || status === null)
      );
    });
...
// Mobile app callback — always redirects to deep link
app.get("/auth/patreon/mobile-callback", async (req, res) => {
  const { code } = req.query;
  try {
    const tokenRes = await axios.post(
      "https://www.patreon.com/api/oauth2/token",
...
app.get("/validate-token", async (req, res) => {
  ...
  try {
    ...
    if (decoded.authType === "license") {
      ...
      // Check if time is up
      const isPastTime =
        lic && lic.expires_at && new Date(lic.expires_at) < new Date();
    ...
    if (decoded.authType === "patreon") {
      if (statusCache.get(`active_${decoded.id}`))
      ...
      if (!active) {
        const memberships = (live.data.included || []).filter(
          (m) => m.type === "member",
        );
